"""
Gera PNGs coloridos do TIFF de Cobertura para renderizacao raster no MapLibre.
Evita a aparencia angular dos poligonos vetoriais ao manter a resolucao do pixel.

Classes (MapaBiomas Col.10, excluindo agricolas e agua/urbanizado):
  9  = Silvicultura
...
Gera:
  public/dados_convertidos/rio_grande/cobertura_raster.png             (BASE)
  public/dados_convertidos/rio_grande/cenarios/
    cobertura_raster_cenario_maio_2024.png
    cobertura_raster_cenario_maio_2024_50.png

Uso: conda run -n climada_env python scripts/converter_cobertura_raster.py
"""
...
# Cores RGB por classe (devem corresponder a COBERTURA_COLORS em page.tsx)
COLORS = {
    9:  (122,  89,   0),   # Silvicultura             — marrom escuro
    11: ( 81, 151, 153),   # Campo Alagado e Pantanosa— azul-esverdeado
    12: (214, 188, 116),   # Formacao Campestre       — caqui
    21: (232, 192, 107),   # Mosaico de Usos          — amarelo queimado
    49: (  2, 214,  89),   # Restinga Arborea         — verde vivo
    50: (173,  81,   0),   # Restinga Herbacea        — laranja escuro
}
OPACITY = 200  # ~78 %
...
print("\n2. Gerando PNG base...")
save_png(band_to_rgba(band_base), OUT_DIR / "cobertura_raster.png")
...
for slug, mancha_path in CENARIOS.items():
    ...
    save_png(band_to_rgba(band_masked), CENARIOS_DIR / f"cobertura_raster_{slug}.png")

# ── Imprimir bounds para colar no page.tsx ────────────────────────────────────

print(f"""
Bounds para page.tsx:
const COBERTURA_RASTER_COORDS: [[number,number],[number,number],[number,number],[number,number]] = [
  [{left:.6f}, {top:.6f}],   // NW
  [{right:.6f}, {top:.6f}],  // NE
